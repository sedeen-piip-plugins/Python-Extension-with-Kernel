    ...
    namespace tile {
      ...
      RawImage PythonKernel::doProcessData(const RawImage &source)
      {
        ...
        try
        {
          ...
          //set the python path
          std::shared_ptr<python::PythonObject> sys(python::PythonObject::py_ImportModule("sys"));
          assert(sys->py_Object() != NULL);
          std::shared_ptr<python::PythonObject> path(sys->py_GetAttr("path"));
          assert(path->py_Object() != NULL);
          PyList_Append(path->py_Object(), PyString_FromString(pathToPythonScript_.c_str()));

          //load the module
          std::shared_ptr<python::PythonObject> pModule(python::PythonObject::py_ImportModule(pyModuleName_.c_str())); //"PathologyAnalyser"
          assert(pModule->py_Object() != NULL);

          // find the class
          std::shared_ptr<python::PythonObject> pClass(pModule->py_GetAttr(python_class_name_.c_str()));
          ...
          std::shared_ptr<python::PythonObject> pInstance(pClass->py_CallObject(python::PythonObject(pArgs, true)));
          assert(pInstance->py_Object() != NULL);
          Py_DECREF(pArgs);

          //get the function
          std::shared_ptr<python::PythonObject> pMethod(pInstance->py_GetAttr("run"));
          assert(pMethod->py_Object() != NULL);

          pArgs = Py_BuildValue("()");
          assert(pArgs != NULL);
          std::shared_ptr<python::PythonObject> pResult(pMethod->py_CallObject(python::PythonObject(pArgs, true)));
      ...
      ColorSpace const &PythonKernel::doGetColorSpace() const
      {
        return color_;
      }
